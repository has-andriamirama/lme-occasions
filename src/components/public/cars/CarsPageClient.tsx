...
const FUEL_OPTIONS = [
  { value: '', label: 'Tous' }, { value: 'GASOLINE', label: 'Essence' },
  { value: 'DIESEL', label: 'Diesel' }, { value: 'ELECTRIC', label: 'Électrique' },
  { value: 'HYBRID', label: 'Hybride' },
]
const TRANS_OPTIONS = [
  { value: '', label: 'Toutes' }, { value: 'MANUAL', label: 'Manuelle' },
  { value: 'AUTOMATIC', label: 'Automatique' },
]
const STATUS_OPTIONS = [
  { value: 'ALL', label: 'Tous' }, { value: 'AVAILABLE', label: 'Disponibles' },
  { value: 'RESERVED', label: 'Réservés' }, { value: 'SOLD', label: 'Vendus' },
]
...
export default function CarsPageClient({ brands }: { brands: string[] }) {
  ...
  const activeFiltersCount = Object.entries(filters).filter(([k, v]) => v && v !== 'ALL' && v !== 'newest' && k !== 'sortBy').length
  ...
  return (
    <div className="pt-24 pb-20 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="font-display font-black text-3xl sm:text-4xl text-white mb-2">Notre catalogue</h1>
          <p className="text-dark-400 text-sm">{meta.total} véhicule{meta.total !== 1 ? 's' : ''} trouvé{meta.total !== 1 ? 's' : ''}</p>
        </div>
              ...
              <div className="flex items-center justify-center gap-2 mt-10">
                {Array.from({ length: meta.totalPages }, (_, i) => i + 1).map((p) => (
                  <button key={p} onClick={() => setPage(p)}
                    className={cn('w-10 h-10 flex items-center justify-center rounded-xl text-sm font-medium transition-all',
                      p === page ? 'bg-brand-500 text-dark-950 shadow-brand' : 'bg-dark-800 text-dark-400 hover:text-white hover:bg-dark-700 border border-dark-700')}>
                    {p}
                  </button>
                ))}
              </div>
