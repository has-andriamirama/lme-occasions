...
const FROM     = process.env.EMAIL_FROM          ?? 'LME Occasions <noreply@lmeoccasions.com>'
...
export async function sendReservationConfirmationToClient(data: {
  ...
  const html = wrap(`
    ...
    <div class="warning">
      ⚠️ <strong>Délai de 5 jours :</strong> En cas de non-présentation avant le ${expiryStr},
...
export async function sendReservationExpiredToClient(data: {
  ...
  return transporter.sendMail({
    from: FROM,
    to: data.clientEmail,
    subject: `⚠️ Réservation expirée — ${data.carTitle}`,
    html,
  })
}
